...
class Reacher2DGCBCDataset(Dataset):
    ...
    def __init__(
        ...
        for task_id, var_id in self.allowed_ids:
            ...
            for ep in episodes[:max_ep_idx]:
                ...
                if self.image_based:
                    img = torch.load(self.data_path / 'torch_imgs' / f'{var_id}_{cond_id}.torch').permute(0, 3, 1, 2) #channel first
    ...
    def __getitem__(self, idx):
        ...
        if self.image_based:
            ...
            g = torch.as_tensor(self.obses[task_idx][state_action_idx][-1])
        else:
