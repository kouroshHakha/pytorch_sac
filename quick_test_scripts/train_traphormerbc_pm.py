...
from osil.nets import TraphormerBCLightningModule
...
class PointmassBCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        context_s = torch.as_tensor(obses, dtype=torch.float)
        context_a = torch.as_tensor(acs, dtype=torch.float)

        return context_s, context_a, x, y
...
def main(pargs):
    ...
    agent = TraphormerBCLightningModule.load_from_checkpoint(ckpt) if ckpt else TraphormerBCLightningModule(config)
