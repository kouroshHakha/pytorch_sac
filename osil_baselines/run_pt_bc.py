...
def main(pargs):
    ...
    ########################################################################
    #  pre-training a bc agent on the entire dataset we have from the domain
    ########################################################################
    exp_name = f'pt_bc_{domain}_{pargs.obs_type}'
    ...
    ft_dset = None
    for task in tasks:
        dataset = DMC(f'{domain}_{task}', pargs.obs_type, block_size=1)
    ...
    eval_cfg.update(normalizer=ft_dset.normalize_returns)
    ...
    print('Evaluating pretrained agent done.')
